...
class LLMService:
    ...
    def chat(self, message: str, history: Optional[list] = None, health_context: Optional[str] = None) -> dict:
    ...
    def analyze(self, health_context: str) -> dict:
        """Sync analyze — delegates to async generate()."""
        prompt = f"Analyze the following health data and provide insights:\n\n{health_context}"
        result = asyncio.run(self._generate_sync(
            {"model": self.default_model, "prompt": prompt, "stream": False},
            self.default_model,
        ))
        return {
            "analysis": result,
    ...
    def suggestions(self, health_context: str) -> dict:
        """Sync suggestions — delegates to async generate()."""
